...
export default function Home() {
  ...
  async function handleProcess() {
    ...
    try {
      ...
      if (!mvhCols.eid || !mvhCols.pen || !mvhCols.eart) {
      ...
      if (useTreatments && treatFile) {
        ...
        const trtCols = Object.fromEntries(
          Object.keys(trtRows[0] || {}).map((c) => [c.toLowerCase(), c])
        );

        if (!trtCols.eart || !trtCols.treatment) {
        ...
        const trtMap = new Map(
          trtRows
            .map((row) => [
              normalizeId(row[trtCols.eart]),
              String(row[trtCols.treatment] ?? '').trim()
            ])
            .filter(([key]) => key)
        );
        ...
        matchedRows = matchedRows.map((row) => ({
          ...row,
          Treatment: trtMap.get(String(row.EART_norm)) || ''
        }));
        ...
        <div style={{ ...sectionCard, position: 'sticky', top: 20 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input
              type="file"
              accept=".csv"
              onChange={(e) => setMvhFile(e.target.files?.[0] || null)}
            />
          </div>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input
                type="checkbox"
                checked={useTreatments}
                onChange={(e) => setUseTreatments(e.target.checked)}
              />
              Use Treatments.xlsx
            </label>
          </div>
          ...
          {useTreatments && (
            <div style={{ marginBottom: 12 }}>
              ...
              <input
                type="file"
                accept=".xlsx,.xls"
                onChange={(e) => setTreatFile(e.target.files?.[0] || null)}
              />
            </div>
          )}
